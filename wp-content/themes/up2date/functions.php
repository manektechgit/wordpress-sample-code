<?php
/**
 * Up2Date functions and definitions
 *
 ...
 * @since Up2Date 1.0
 */
function up2date_setup() {
	/*
	 * Makes Up2Date available for translation.
	 *
	 * Translations can be filed at WordPress.org. See: https://translate.wordpress.org/projects/wp-themes/revolution
	 */
	load_theme_textdomain( 'up2date' );

	// This theme styles the visual editor with editor-style.css to match the theme style.
	add_editor_style( array( 'assets/css/admin/editor-style.css', up2date_fonts_url() ) );


	/**
	 * Adding Primary & Footer menu location for wp_nav_menu
	 */
	register_nav_menu( 'primary', __( 'Primary Menu', 'up2date' ) );
	register_nav_menu( 'footer', __( 'Footer Menu', 'up2date' ) );
...
add_action( 'after_setup_theme', 'up2date_setup' );

/**
 * Register custom fonts.
 */
function up2date_fonts_url() {
	$fonts_url = '';
 
	$libre_franklin = _x( 'on', 'Libre Franklin font: on or off', 'up2date' );
 ...
 * @since Up2Date 1.0
 */
function up2date_scripts_styles() {
	global $wp_styles;
	...
	wp_register_script('jquery', JS_PATH.'jquery.min.js', false, null);
    ...
    //Enqueue CSS Files
	wp_enqueue_style( 'google-fonts', 'https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@100;200;300;400;500;600;700&display=swap' );
	wp_enqueue_style( 'fontawesome', get_template_directory_uri().'/assets/vendor/fontawesome/css/all.css' );
	wp_enqueue_style( 'bootstrap', get_template_directory_uri().'/assets/vendor/bootstrap/css/bootstrap.min.css' );
	wp_enqueue_style( 'swiper', CSS_PATH.'swiper-bundle.min.css' );
	...
	wp_enqueue_script( 'bootstrap4', get_template_directory_uri().'/assets/js/bootstrap.min.js', array(), $theme_version, true );
	wp_enqueue_script( 'swiper-bundle', JS_PATH.'swiper-bundle.min.js', array(), $theme_version, true );
	wp_enqueue_script( 'custom', JS_PATH.'custom.js', array(), $theme_version, true );
	wp_localize_script('custom', 'up2date_general', $translation_array);
	
	wp_enqueue_style( 'up2date-ie', CSS_PATH.'admin/ie.css' );
	if ( ( ! is_admin() ) && is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
	$wp_styles->add_data( 'up2date-ie', 'conditional', 'lt IE 9' );
	
	// Loads our main stylesheet.
	wp_enqueue_style( 'up2date-style', get_stylesheet_uri() );
	
}
add_action( 'wp_enqueue_scripts', 'up2date_scripts_styles' );
 ...
 */
function up2date_resource_hints( $urls, $relation_type ) {
	if ( wp_style_is( 'up2date-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
...
add_filter( 'wp_resource_hints', 'up2date_resource_hints', 10, 2 );
 ...
 */
function up2date_wp_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() )
		$title = "$title $sep " . sprintf( __( 'Page %s', 'up2date' ), max( $paged, $page ) );

	return $title;
}
add_filter( 'wp_title', 'up2date_wp_title', 10, 2 );
 ...
 * @since Up2Date 1.0
 */
function up2date_page_menu_args( $args ) {
	if ( ! isset( $args['show_home'] ) )
		$args['show_home'] = true;
	return $args;
}
add_filter( 'wp_page_menu_args', 'up2date_page_menu_args' );
 ...
 * @since Up2Date 1.0
 */
function up2date_widgets_init() {
	register_sidebar( array(
		'name' => __( 'Main Sidebar', 'up2date' ),
		'id' => 'sidebar-1',
		'description' => __( 'Appears on posts and pages except the optional Front Page template, which has its own widgets', 'up2date' ),
	...
	if( is_plugin_active( ACF_PLUGIN_CHECK ) ):
		if(options('show_footer_widget_area') == 1) :
			...
			for($i=1;$i<=$count;$i++)
			{
				...
				$w_id = 'footer-'.$i;
				register_sidebar( array(
					'name' => __( $w_name, 'up2date' ),
					'id' => $w_id ,
					'description' => __( 'Appears when using Footer widget Enables', 'up2date' ),
					'before_widget' => '<aside id="%1$s" class="widget %2$s">',
					'after_widget' => '</aside>',
					'before_title' => '<h3 class="widget-title">',
					'after_title' => '</h3>',
...
add_action( 'widgets_init', 'up2date_widgets_init' );


if ( ! function_exists( 'up2date_content_nav' ) ) :
/**
 * Displays navigation to next/previous pages when applicable.
 *
 * @since Up2Date 1.0
 */
function up2date_content_nav( $html_id ) {
	global $wp_query;

	if ( $wp_query->max_num_pages > 1 ) : ?>
		<nav id="<?php echo esc_attr( $html_id ); ?>" class="navigation" role="navigation">
			<h3 class="assistive-text"><?php _e( 'Post navigation', 'up2date' ); ?></h3>
			<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'up2date' ) ); ?></div>
			<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'up2date' ) ); ?></div>
		</nav><!-- .navigation -->
	<?php endif;
}
endif;

if ( ! function_exists( 'up2date_comment' ) ) :
/**
 * Template for comments and pingbacks.
 *
 * To override this walker in a child theme without modifying the comments template
 * simply create your own up2date_comment(), and that function will be used instead.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 *
 * @since Up2Date 1.0
 */
function up2date_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
		<p><?php _e( 'Pingback:', 'up2date' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( '(Edit)', 'up2date' ), '<span class="edit-link">', '</span>' ); ?></p>
	<?php
			break;
		default :
			...
			<header class="comment-meta comment-author vcard">
				<?php
					...
					printf( '<cite><b class="fn">%1$s</b> %2$s</cite>',
						...
						( $comment->user_id === $post->post_author ) ? '<span>' . __( 'Post author', 'up2date' ) . '</span>' : ''
					);
					printf( '<a href="%1$s"><time datetime="%2$s">%3$s</time></a>',
						esc_url( get_comment_link( $comment->comment_ID ) ),
						get_comment_time( 'c' ),
						/* translators: 1: date, 2: time */
						sprintf( __( '%1$s at %2$s', 'up2date' ), get_comment_date(), get_comment_time() )
					);
				?>
			</header><!-- .comment-meta -->

			<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'up2date' ); ?></p>
			<?php endif; ?>

			<section class="comment-content comment">
				<?php comment_text(); ?>
				<?php edit_comment_link( __( 'Edit', 'up2date' ), '<p class="edit-link">', '</p>' ); ?>
			</section><!-- .comment-content -->

			<div class="reply">
				<?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Reply', 'up2date' ), 'after' => ' <span>&darr;</span>', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
			</div><!-- .reply -->
		</article><!-- #comment-## -->
	<?php
		break;
...
if ( ! function_exists( 'up2date_entry_meta' ) ) :
/**
 * Set up post entry meta.
 *
 * Prints HTML with meta information for current post: categories, tags, permalink, author, and date.
 *
 * Create your own up2date_entry_meta() to override in a child theme.
 *
 * @since Up2Date 1.0
 */
function up2date_entry_meta() {
	// Translators: used between list items, there is a space after the comma.
	$categories_list = get_the_category_list( __( ', ', 'up2date' ) );

	// Translators: used between list items, there is a space after the comma.
	$tag_list = get_the_tag_list( '', __( ', ', 'up2date' ) );
	...
	$author = sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
		...
		esc_attr( sprintf( __( 'View all posts by %s', 'up2date' ), get_the_author() ) ),
		get_the_author()
	);

	// Translators: 1 is category, 2 is tag, 3 is the date and 4 is the author's name.
	if ( $tag_list ) {
		$utility_text = __( 'This entry was posted in %1$s and tagged %2$s on %3$s<span class="by-author"> by %4$s</span>.', 'up2date' );
	} elseif ( $categories_list ) {
		$utility_text = __( 'This entry was posted in %1$s on %3$s<span class="by-author"> by %4$s</span>.', 'up2date' );
	} else {
		$utility_text = __( 'This entry was posted on %3$s<span class="by-author"> by %4$s</span>.', 'up2date' );
	}
 ...
 */
function up2date_body_class( $classes ) {
	...
	if ( wp_style_is( 'up2date-fonts', 'queue' ) )
...
add_filter( 'body_class', 'up2date_body_class' );
 ...
 * @since Up2Date 1.0
 */
function up2date_content_width() {
	if ( is_page_template( 'page-templates/full-width.php' ) || is_attachment() || ! is_active_sidebar( 'sidebar-1' ) ) {
		global $content_width;
		$content_width = 960;
	}
}
add_action( 'template_redirect', 'up2date_content_width' );
 ...
 */
function up2date_customize_register( $wp_customize ) {
	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial( 'blogname', array(
			...
			'render_callback' => 'up2date_customize_partial_blogname',
		) );
		$wp_customize->selective_refresh->add_partial( 'blogdescription', array(
			'selector' => '.site-description',
			'container_inclusive' => false,
			'render_callback' => 'up2date_customize_partial_blogdescription',
		) );
	}
}
add_action( 'customize_register', 'up2date_customize_register' );

/**
 * Render the site title for the selective refresh partial.
 *
 * @since Up2Date 1.0
 * @see up2date_customize_register()
 *
 * @return void
 */
function up2date_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 * @return void
 */
function up2date_customize_partial_blogdescription() {
 ...
 * @since Up2Date 1.0
 */
function up2date_customize_preview_js() {
	wp_enqueue_script( 'up2date-customizer', JS_PATH.'theme-customizer.js', array( 'customize-preview' ), '20141120', true );
}
//add_action( 'customize_preview_init', 'up2date_customize_preview_js' );
 ...
 */
function up2date_mime_types($mimes) {
  $mimes['svg'] = 'image/svg+xml';
  return $mimes;
}
add_filter('upload_mimes', 'up2date_mime_types');
...
if( is_plugin_active( ACF_PLUGIN_CHECK ) ):
	...
	{ 
		...
		// Button Color
		if($btn_color != ''){$btn_color = $btn_color ;}
		else{ $btn_color = '#cf202f'; } 
		
		// Label Color
		if($label_color != ''){$label_color = $label_color ;}
		else{ $label_color = '#182A54'; } 
		?>
		    <style type="text/css">
	...
	function logo_header($use_dark = FALSE) {
		...
		echo '<div class="logo-wrap">';
			if($logo != ''):
				if($use_dark == TRUE && $dark_logo != ''):
					echo '<a href="'. esc_url( home_url( '/' ) ).'"><img src="'.$dark_logo.'" alt="'.$blogName.'" title="'.$blogName.'" /></a>';
				else:
			...
			if(!empty($site_loading_image)):
				echo '<div class="siteloader">
				        <div class="loader-content">
				            <img src="'.esc_url($site_loading_image).'" alt="loading-icon" />
				        </div>
				    </div>';
			else:
				echo '<div class="siteloader">
	...
	function social_list() {			
		...
		if( have_rows('social_links', 'option') ):
			...
			while ( have_rows('social_links', 'option') ) : the_row();
				...
				$mediaClass = get_sub_field('icon_class', 'option');
...
function flexLayout($attr){
	...
	if ( !empty($fields[$flex_field]) && isset($fields[$flex_field]['ID']) && $fields[$flex_field]['ID']!='')
	{
